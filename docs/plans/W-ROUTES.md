...
## 단계 7 리뷰 대응 R2 — `[P1]` 이의를 발행해도 목록이 갱신되지 않는다

```
Locator: heading '이의' … ul.record-list > li
Expected: 1     Received: 0
```

B가 이의를 발행했는데 **그 화면의 목록이 비어 있다.**
사용자가 방금 올린 자기 이의를 보지 못한다.

R6에서 자동 로드를 넣었지만 **발행 성공 후 다시 부르지 않는다.**

### 고칠 것

`challenge.tsx`에서 발행(그리고 철회)에 성공하면 **목록을 다시 불러온다.**

- 이미 있는 `load()`를 성공 핸들러에서 호출한다.
- `onSuccess` prop은 상위(상세)의 상태 인장 갱신용이므로 **그대로 두고**,
  자기 목록 갱신은 컴포넌트 안에서 한다.
- 발행 직후 체인에 아직 반영되지 않을 수 있으므로 영수증을 받은 뒤에 부른다.

같은 문제가 **정산 발행·철회 후에도 있는지 확인**할 것 —
`settlement.tsx`에서 발행하면 상세의 정산 절이 갱신되는가.
