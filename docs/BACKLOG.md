...
> **W1~W8은 브랜치 `feat/w-frontend`에만 있다.** 무인 규칙상 main에 병합하지 않았다 —
> 프론트는 사람이 보고 판단해야 한다. 디자인은 하지 않았다(기능이 보이는 최소 마크업).
...
| ID | P | 항목 | 완료 조건 |
|---|---|---|---|
| W1 | **P0** | F1 지갑 연결 + `isVerified` + `verifiedAddressUID` 스냅샷 | `[~]` 브랜치 `feat/w-frontend`. 3회 백오프 후 "확인 불가" · 미검증 지갑 사용 허용. ⚠️ **`verifiedAddressUID` 미해결** — Dojang이 UID getter를 노출하지 않는다. 검증 스키마 UID를 알아야 EAS 로그로 찾는다. 현재 0으로 기록하고 화면에 명시 |
| W2 | **P0** | F2·F3 3계층 등록 (저널→노트→결정) | `[~]` 브랜치. 저널은 localStorage만 · 문구 상시 표시 · 노트 승격 |
| W3 | **P0** | **salt 백업 강제 유도** | `[~]` 브랜치. 백업 확인 전 발행 버튼 비활성 · salt를 서버·localStorage에 두지 않음 |
| W4 | **P0** | F4 결정 커밋 | `[~]` 브랜치. commitment 4종 · 평문으로 나가는 항목을 화면에 명시 · trigger 강제 불가 표시 · 제출 전 검증(부모 9개·과거 window·grace) |
| W5 | **P0** | F6 정산 발행 | `[~]` 브랜치. `observedAt`을 windowEnd로 고정(입력 안 받음) · `result`를 고르게 하지 않고 계산 · `supersedes` 자동 · X7 한국어 오류 |
| W6 | **P0** | F7 Challenge 발행 + 목록 | `[~]` 브랜치. 건수 미표시·정렬 없음·검증 지갑 병기·철회된 이의 제외. 변이 테스트로 정렬/철회포함이 잡히는 것 확인 |
| W7 | **P0** | F8 상태 표시 | `[~]` 브랜치. core `deriveState` 재사용 · 철회 이력을 상태와 분리해 병기 · `now` 1초 갱신으로 OVERDUE 전환이 화면에 보임 |
| W8 | **P0** | **F5 Reveal** (B10) | `[~]` 브랜치. 클라이언트 재계산 대조 · 불일치 시 다운로드 차단 · `reveals/README.md`. 자동 업로드는 만들지 않음(토큰 범위 밖) |
