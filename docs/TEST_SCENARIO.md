...
- 기대 화면: 쪽빛 `정산완료` 인장, 등급의 두 축, `커밋`, `예상 결과`, `정산`,
...
- 기대 화면: `정산완료` 인장 아래 「정산 철회 이력 있음」이 보이고,
  `▸ 이전 정산 (철회됨)`을 펼칠 수 있다.
...
- 기대 화면: 처음 `대기`이고, 경계를 지나며 `관측중` → `정산대기` →
  `기한초과`로 바뀐다.
...
S8에서 결정을 발행한 뒤 정산 가능 시각까지 체인 시간을 옮겨 A로 정산하고,
다른 지갑 B로 이의를 발행한다.

- 기대 화면: 발행 성공 뒤 새 상세 URL로 자동 이동한다. 활성 정산이 나타나고,
  지갑을 B로 바꾸면 그 정산에 이의를 발행할 수 있다.
