...
- **O1** — GIWA Sepolia 테스트넷 가스가 없으면 배포 자체가 불가능하다. **오늘 확보한다.**
...
| 게이트 | 시점 | 조건 | 검증 명령 |
|---|---|---|---|
| **G0** | D-0 (7/27) | 배포 지갑에 가스 확보 + 법률 검토 게이트(B14) 확인 | `cast balance $DEPLOYER --rpc-url giwa_sepolia` |
| **G1** | D-0 | `_decodeDecision` 단위 테스트 통과 | `forge test --mt test_DecodeDecision -vv` |
| **G2** | D-0 | commitment 테스트 벡터 고정 (컨트랙트·TS 양쪽 동일값) | `forge test --mt test_CommitmentVector` + `pnpm -C core test` |
...
```
D-0  7/27  O1 가스 확보 · O2 법률 게이트 · X1 벡터 고정 · C1 디코딩 트릭 · C2 Base
D+1  7/28  C3~C6 리졸버 · C7 공격 테스트 19종 · O3 배포 · O4 fixture(★즉시) · O5 metric 등록
...
| R2 | 테스트넷 가스 미확보 | 전부 정지 | G0. 실패 시 즉시 대안 파우셋 조사 |
| R3 | EAS `1.4.1-beta.3` ↔ lib `v1.4.0` 미묘한 차이 | 배포 후 revert | C8 — 배포 전 `getAttestation`·`attest` 셀렉터를 온체인과 대조 |
