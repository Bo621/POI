...
| 항목 | 명세 | MVP | 근거 |
|---|---|---|---|
| metric | 6종 (§11.2) | **2종** — `BTC_PRICE_KRW_AT_END` · `BTC_MAX_DRAWDOWN_IN_WINDOW` | 지표 하나마다 계산식·출처·간격·UTC·결측치·스냅샷 해시를 정의한 문서가 필요하고(§11.3), 그 문서 해시가 `definitionHash`다. 문서 없는 지표는 컨트랙트가 등록을 **거부**하므로 미등록 상태가 안전하다. 지표는 append-only라 Phase 1에 추가 가능하다 |
