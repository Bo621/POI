...
**피치덱 재업로드 하나.** `submit/latest/POI_pitchdeck_v2.4_2026-07-30.pdf` 를
기존 Drive 파일에 「새 버전 업로드」로 올린다 (링크 유지). v2.3 에서 판정 서술 과장과
시장 수치를 고쳤다. 팀 소개는 이미 일치하므로 손대지 않는다.
...
연락처·생년월일만 양식에 직접 적으면 된다. 저장소에는 넣지 않는다.
...
> 이전 버전 바이트를 그대로 승계**할 것 (v2.1·v2.2 에서 그렇게 했다).
