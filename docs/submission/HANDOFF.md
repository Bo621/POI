...
**제출 완료 (2026-07-31).** Track 03 GIWA-NATIVE IDEAS.
제출본은 `submit/v3.7_2026-07-31/` 이고 Drive 업로드본과 2/2 해시가 일치한다.

---

## 1. 지금 당장 해야 할 것

**없다 — 제출이 끝났다.** 아래는 제출 시점의 기록이다. 문항 1~12 값이 확정됐고 Drive 업로드본도 로컬 PDF 와 해시가 일치한다
**Drive 교체 완료** — 2026-07-31 쿠키 없이 받아 2/2 해시 일치를 확인했다.
> 제출 후 세 번 갱신했다 — 팀 소개(회사명·직책·수상 정리) · 피치덱(데모 결정 교체) ·
> 피치덱(도장 검증 붙인 결정으로 재교체).
> **최종: 피치덱 `b02b52d8…` 20쪽 · 팀 소개 `4d1b6bdf…` 2쪽.**
> 해시의 유일한 원장은 `submit/latest/SHA256SUMS.txt` 다.
