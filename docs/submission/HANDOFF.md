...
(피치덱 `0322deaa…` 20쪽 · 팀 소개 `86cbc8ad…` 2쪽).
...
연락처·생년월일만 폼에 직접 적는다. **이 저장소는 공개라 값을 커밋하지 않는다.**
값은 `submit/PRIVATE_폼입력값.txt` 에 있다 — `.gitignore` 로 막혀 있어 푸시되지 않는다.
다른 세션에서 이어받는데 그 파일이 없으면, 사용자에게 직접 물어볼 것.
