...
nav 오른쪽에 `GIWA Sepolia · 91342` 가 보인다. **집계 숫자·비율·적중률은 어디에도 없다.**
...
| 인장 | UID | 무엇을 보여주나 |
|---|---|---|
| 🔴 기한초과 | `0x3f592f21…e16938` | **미발행이 드러난다** |
| 🔵 등록완료 | `0x5941a398…443888` | 결과 등록 + **제3자 이의** |
| 🔵 등록완료 | `0xb1e46283…683b21` | **결과 등록 철회 이력 있음** |

상세에서 확인할 것:

- `커밋` — 발행자 주소가 링크다. 누르면 그 지갑의 다른 판단으로 간다
- `예상 결과` — 조건이 `> 1`, 관측 구간이 UTC 로 명시
- `결과 등록` — 관측값 `91688000`, 출처 `upbit-1m-candles`
...
| S3 오프체인 재현 | ✅ `MATCH` · 종료코드 0 · 재계산값 `91688000` = 온체인 |
...
위 열두 시나리오 중 하나라도 실행해서 실패하면 그건 결함이다.
