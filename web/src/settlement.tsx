...
function errorMessage(error: unknown): string {
    ...
    return typeof data === "string" && isHex(data)
        ? messageFromRevert(data) ?? "결과 등록 발행에 실패했습니다."
        : error instanceof Error ? error.message : "결과 등록 발행에 실패했습니다.";
}
...
export function Settlement({address, decisionUID, onSuccess}: {
    ...
    async function prepare(event: FormEvent) {
        ...
        try {
            ...
            setPrepared(payload);
            if (!payload.canPublish) setStatus("먼저 기존 결과 등록을 철회해야 합니다.");
        } catch (error) {
    ...
    async function revoke() {
        ...
        try {
            ...
            if (heads.activeHead === ZERO_UID) throw new Error("활성 결과 등록이 없습니다.");
    ...
    return (
        <section>
            ...
            {prepared && prepared.supersedes !== ZERO_UID && <p>철회된 이전 등록을 정정합니다 <span className="hex">{prepared.supersedes}</span></p>}
            {!address && <p className="notice notice--quiet">지갑을 연결해야 결과 등록을 발행할 수 있습니다.</p>}
            <div className="button-row">
                <button className="btn-commit" type="button" onClick={publish} disabled={!address || !prepared?.canPublish || !isDeployed()}>등록 발행</button>
                <button className="btn-quiet" type="button" onClick={revoke} disabled={!prepared || !isDeployed()}>등록 철회</button>
            </div>
            ...
            <Receipt label="결과 등록" uid={receipt?.uid} txHash={receipt?.txHash} />
            <Receipt label="등록 철회" txHash={revokeTxHash} />
        </section>
    );
}
