...
### `지표 조회 실패: 업비트 HTTP 429` 가 나오면

업비트 공개 API 는 IP 당 호출 한도가 있습니다. **POI 쪽 실패가 아닙니다** —
종료코드 `2`(조회 실패)이지 `1`(불일치)이 아닙니다. 잠시 뒤 다시 실행하면 됩니다.

기다리지 않고 온체인 쪽만 보려면 `--no-fetch` 를 붙입니다.
재계산을 하지 않으므로 판정은 `NO_OBSERVATION`(종료코드 `3`)이 되지만,
정산에 무엇이 기록돼 있는지는 그대로 보입니다.

```bash
node --experimental-strip-types verifier/src/cli.ts \
  0x5941a398a8338b99d053309cbf5e611486f30e649c9569cfa3a63d5060443888 --json --no-fetch
```
