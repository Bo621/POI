...
두 건입니다. **문서가 인용하는 것은 ②** 입니다.

| | 결정 | 도장 | 관측값 | 판정 |
|---|---|---|---|---|
| ① | `0x5e341843…353f3d` | 없음 (미검증 지갑) | 91,179,000 | 맞음 |
| ② | `0xe7fe5923…fc2da0` | **TESTNET FAUCET** | 91,020,000 | 맞음 |

①을 지우지 않고 남겨 둡니다. **두 건을 나란히 열면 도장이 붙은 화면과 안 붙은 화면의
차이가 그대로 보입니다.** 기록을 지우지 않는 것이 이 제품의 전제이기도 합니다.

```
② 술어   BTC_PRICE_KRW_AT_END  >  90,000,000   (op GT · decimals 0 · frozen)
   구간   1785486145 ~ 1785486745  (600초)
   관측   91,020,000  ·  upbit-1m-candles  ·  observedAt = windowEnd
   판정   result 0 (맞음) — 컨트랙트가 I17 로 재계산해 강제
   스냅샷 0xc1f5746c…4bb5  (TESTNET FAUCET · 2026-08-28 만료)
```
...
② 커밋 tx `0x916618cf…db109c` · 정산 tx `0xecef1f35…1f03b31`
검증기 재현 — 둘 다 `MATCH` · 종료코드 0
