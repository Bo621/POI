...
| **t1** 구간 종료 | 업비트 공개 1분봉으로 관측값이 확정됩니다 — **91,020,000** |
| | 공개 데이터라 **누구나 같은 값을 다시 구할 수 있습니다** |
| **t2** 결과 등록 | 발행자가 낸 관측값으로 **컨트랙트가 다시 계산합니다.** `91,020,000 > 90,000,000` 이므로 「맞음」 |
...
> **화면의 「도장 검증 — TESTNET FAUCET」이 GIWA 도장 연동입니다.**
> 커밋할 때 도장 스냅샷 UID 를 함께 넣으면 컨트랙트가 **스키마와 발급자를 둘 다 검증**하고
> 통과시킵니다. 「누가 말했는가」가 이렇게 붙습니다.
> 받아 본 것은 파우셋 발급분이고 KYC 경로는 아직 검증하지 못했습니다 —
> 그래서 화면도 발급자 라벨을 뭉뚱그리지 않고 그대로 씁니다.
